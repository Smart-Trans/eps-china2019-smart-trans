...
def read_and_prepare_data(file_path):
    ...
    Eprocessed_data = pd.DataFrame(index=year_columns)
    ...
    energy_sectors = {
        "Agricultural": "BAU Industrial Fuel Use for Energy Purposes.*agriculture",
        "Iron and Steel": "BAU Industrial Fuel Use for Energy Purposes.*iron and steel",
        "Petrochem & Coke": "BAU Industrial Fuel Use for Energy Purposes.*petroleum and coke",
        "Non-metallic Mineral": "BAU Industrial Fuel Use for Energy Purposes.*cement and other nonmetallic minerals",
        "Chemicals": "BAU Industrial Fuel Use for Energy Purposes.*chemicals",
        "Mining and Washing of Coal": "BAU Industrial Fuel Use for Energy Purposes.*coal mining",
        "Construction": "BAU Industrial Fuel Use for Energy Purposes.*construction",
    }
    
    #计算工业非氢部门
    industryE_data=data[data['Time'].str.contains("BAU Industrial Fuel Use for Energy Purposes") & ~data['Time'].str.contains("agriculture") & ~data['Time'].str.contains("Change") & ~data['Time'].str.contains("Output") & ~data['Time'].str.contains("before")]
    industryE=industryE_data[year_columns].apply(pd.to_numeric, errors='coerce').fillna(
        0).sum() * 3.60087861438191E-12
    Eprocessed_data["Industry Overall"]=industryE
    #得到分行业能源消费
    for sector_key, pattern in energy_sectors.items():
        # 确保行不包含“BAU”
        sector_data = data[data['Time'].str.contains(pattern, regex=True) & ~data['Time'].str.contains("Output") & ~data['Time'].str.contains("Change") & ~data['Time'].str.contains("before")]
        Eprocessed_data[sector_key] = sector_data[year_columns].apply(pd.to_numeric, errors='coerce').fillna(
            0).sum() *3.60087861438191E-12
    #计算分行业增加值
    industryCO2_data=data[data['Time'].str.contains("BAU Industrial Sector Pollutant Emissions") & data['Time'].str.contains("CO2") & ~data['Time'].str.contains("agriculture")]
    #筛选工业各行业代码
    code=[]
    ...
    for sector in sectors:
        processed_data[sector]=Eprocessed_data[sector]/Pprocessed_data[sector]
...
def plot_sector_energy_consumption(processed_data, output_folder, language):
    if language=='en':
        processed_data=processed_data* 6.5196001920818 *1E-8/(3.60087861438191*1E-12)/1000
    titles = {'en': 'Energy Intensity by Industrial Sector', 'cn': '行业能源消费强度'}
    ylabels = {'en': 'bBTU/millionUSD', 'cn': '吨标煤/万元'}
    ...
    select_data = processed_data.drop(columns=['Agricultural', 'Construction'])
    ...
    output_path = f"{output_folder}/图11：行业能源消费强度{language}无标题.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"{titles[language]} chart saved to: {output_path}")

    plt.title(titles[language],size=18)
    output_path = f"{output_folder}/图11：行业能源消费强度{language}.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"{titles[language]} chart saved to: {output_path}")
