...
def read_and_prepare_data(file_path):
    ...
    sectors = {
        "LDVs": "BAU Cargo Dist Transported by Vehicle Technology.*LDVs.*freight.",
        "HDVs": "BAU Cargo Dist Transported by Vehicle Technology.*HDVs.*freight.",
        "Aircrafts": "BAU Cargo Dist Transported by Vehicle Technology.*aircraft.*freight.",
        "Rail": "BAU Cargo Dist Transported by Vehicle Technology.*rail.*freight.",
        "Ships": "BAU Cargo Dist Transported by Vehicle Technology.*ships.*freight.",
        "Motorbikes": "BAU Cargo Dist Transported by Vehicle Technology.*motorbikes.*freight."
    }
...
def plot_sector_energy_consumption(processed_data, output_folder, language):
    titles = {'en': 'Freight turnover', 'cn': '货运周转量'}
    ylabels = {'en': 'billion t-mile', 'cn': '亿吨公里'}
    ...
    output_path = f"{output_folder}/图n：货运周转量{language}无标题.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"{titles[language]} chart saved to: {output_path}")
    
    plt.title(titles[language],size=18)
    output_path = f"{output_folder}/图n：货运周转量{language}.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"{titles[language]} chart saved to: {output_path}")
