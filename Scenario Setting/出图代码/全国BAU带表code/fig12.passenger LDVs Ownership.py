...
def read_and_prepare_data(file_path):
    ...
    bau_sectors = {
        "Gasoline": "BAU Vehicles.*LDVs.*passenger.*gasoline vehicle.",
        "Diesel": "BAU Vehicles.*LDVs.*passenger.*diesel vehicle.",
        "Natural Gas": "BAU Vehicles.*LDVs.*passenger.*natural gas vehicle.",
        "LPG": "BAU Vehicles.*LDVs.*passenger.*LPG vehicle.",
        "PHV": "BAU Vehicles.*LDVs.*passenger.*plugin hybrid vehicle.",
        "BEV": "BAU Vehicles.*LDVs.*passenger.*battery electric vehicle.",
        "Hydrogen": "BAU Vehicles.*LDVs.*passenger.*hydrogen vehicle."
    }
    ...
    for sector, pattern in start_year_sectors.items():
        start_year_data = data[data['Time'].str.contains(pattern, regex=True)]
    ...
    for sector, pattern in bau_sectors.items():
        bau_data = data[data['Time'].str.contains(pattern, regex=True)]
...
def plot_sector_energy_consumption(processed_data, output_folder, language):
    ...
    max_value = data_for_plotting.sum(axis=1).max()
    ...
    output_path = f"{output_folder}/图12：载客轻型车保有量变化趋势{language}无标题.png"
    #plt.tight_layout()
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"{titles[language]} chart saved to: {output_path}")
    
    plt.title(titles[language], size=18)
    output_path = f"{output_folder}/图12：载客轻型车保有量变化趋势{language}.png"
    #plt.tight_layout()
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"{titles[language]} chart saved to: {output_path}")
