...
def read_and_prepare_data(file_path):
    ...
    sectors = {
        "Industrial": "BAU Industrial Fuel Use for Energy Purposes.*heat.",
        "Urban Residential": "BAU Building Components Energy Use.*urban residential.*heat.",
        "Rural Residential": "BAU Building Components Energy Use.*rural residential.*heat.",
        "Commercial": "BAU Building Components Energy Use.*commercial.*heat.",
    }

    for sector_key, pattern in sectors.items():
        sector_data = data[data['Time'].str.contains(pattern, regex=True) & ~data['Time'].str.contains("before")]
...
def plot_sector_energy_consumption(processed_data, output_folder, language):
    ...
    output_path = f"{output_folder}/图10：分部门集中供热需求量{language}无标题.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"{titles[language]} chart saved to: {output_path}")
    
    plt.title(titles[language],size=18)
    output_path = f"{output_folder}/图10：分部门集中供热需求量{language}.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"{titles[language]} chart saved to: {output_path}")
