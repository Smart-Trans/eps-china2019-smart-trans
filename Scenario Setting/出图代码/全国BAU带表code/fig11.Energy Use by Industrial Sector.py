...
def read_and_prepare_data(file_path):
    ...
    sectors = {
        ...
        "Non-ferrous Metals": "BAU Industrial Fuel Use for Energy Purposes.*other metals.",
        "Chemicals": "BAU Industrial Fuel Use for Energy Purposes.*chemicals.",
        "Rubber and Plastics Products": "BAU Industrial Fuel Use for Energy Purposes.*rubber and plastic products.",
    }

    for sector_key, pattern in sectors.items():
        sector_data = data[data['Time'].str.contains(pattern, regex=True) & (~data['Time'].str.contains("before CCS"))]
        processed_data[sector_key] = sector_data[year_columns].apply(pd.to_numeric, errors='coerce').fillna(0).sum() * 3.60087861438191E-12

    total_industrial_emissions = data[data['Time'].str.contains("BAU Industrial Fuel Use for Energy Purposes") & (~data['Time'].str.contains("before CCS"))][year_columns].apply(pd.to_numeric, errors='coerce').fillna(0).sum() * 3.60087861438191E-12
...
def plot_sector_energy_consumption(processed_data, output_folder, language):
    ...
    for sector in processed_data.columns:
        ...
        axs[1].fill_between(processed_data.index.astype(float), cumulative, cumulative + sector_values, color=colors[sector], label=label)
    ...
    output_path = f"{output_folder}/图11：分行业燃料消费{language}无标题.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"{titles[language]} chart saved to: {output_path}")
    
    plt.title(titles[language],size=18)
    output_path = f"{output_folder}/图11：分行业燃料消费{language}.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"{titles[language]} chart saved to: {output_path}")
