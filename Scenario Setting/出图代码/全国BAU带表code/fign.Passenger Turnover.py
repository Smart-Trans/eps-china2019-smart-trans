...
def read_and_prepare_data(file_path):
    ...
    sectors = {
        "LDVs": "BAU Cargo Dist Transported by Vehicle Technology.*LDVs.*passenger.",
        "HDVs": "BAU Cargo Dist Transported by Vehicle Technology.*HDVs.*passenger.",
        "Aircrafts": "BAU Cargo Dist Transported by Vehicle Technology.*aircraft.*passenger.",
        "Rail": "BAU Cargo Dist Transported by Vehicle Technology.*rail.*passenger.",
        "Ships": "BAU Cargo Dist Transported by Vehicle Technology.*ships.*passenger.",
        "Motorbikes": "BAU Cargo Dist Transported by Vehicle Technology.*motorbikes.*passenger."
    }
...
def plot_sector_energy_consumption(processed_data, output_folder, language):
    ...
    output_path = f"{output_folder}/图n：客运周转量{language}无标题.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"{titles[language]} chart saved to: {output_path}")
    
    plt.title(titles[language],size=18)
    output_path = f"{output_folder}/图n：客运周转量{language}.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"{titles[language]} chart saved to: {output_path}")
