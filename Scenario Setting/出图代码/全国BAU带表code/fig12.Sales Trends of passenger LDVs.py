...
def read_and_prepare_data(file_path):
    ...
    sectors = {
        "Gasoline": "BAU New Vehicles.*LDVs.*passenger.*gasoline vehicle.",
        "Diesel": "BAU New Vehicles.*LDVs.*passenger.*diesel vehicle.",
        "Natural Gas": "BAU New Vehicles.*LDVs.*passenger.*natural gas vehicle.",
        "LPG": "BAU New Vehicles.*LDVs.*passenger.*diesel vehicle.",
        "Plugin Hybrid": "BAU New Vehicles.*LDVs.*passenger.*LPG vehicle.",
        "Battery Electric": "BAU New Vehicles.*LDVs.*passenger.*battery electric vehicle.",
        "Hydrogen":"BAU New Vehicles.*LDVs.*passenger.*hydrogen vehicle."
    }

    for sector_key, pattern in sectors.items():
        sector_data = data[data['Time'].str.contains(pattern, regex=True) &~data['Time'].str.contains("before") &~data['Time'].str.contains("after") &~data['Time'].str.contains("by") &~data['Time'].str.contains("Change") &~data['Time'].str.contains("Last") &~data['Time'].str.contains("After")]
...
def plot_sector_energy_consumption(processed_data, output_folder, language):
    ...
    output_path = f"{output_folder}/图12：载客轻型车新销量变化趋势{language}无标题.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"{titles[language]} chart saved to: {output_path}")
    
    plt.title(titles[language], size=18)
    output_path = f"{output_folder}/图12：载客轻型车新销量变化趋势{language}.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"{titles[language]} chart saved to: {output_path}")
