...
def read_and_prepare_data(file_path):
    ...
    CO2_sectors = {
        "Agricultural": "BAU Industrial Sector Pollutant Emissions.*agriculture.*CO2",
        "Iron and Steel": "BAU Industrial Sector Pollutant Emissions.*iron and steel.*CO2",
        "Petrochem & Coke": "BAU Industrial Sector Pollutant Emissions.*petroleum and coke.*CO2",
        "Non-metallic Mineral": "BAU Industrial Sector Pollutant Emissions.*cement and other nonmetallic minerals.*CO2",
        "Chemicals": "BAU Industrial Sector Pollutant Emissions.*chemicals.*CO2",
        "Mining and Washing of Coal": "BAU Industrial Sector Pollutant Emissions.*coal mining.*CO2",
        "Construction": "BAU Industrial Sector Pollutant Emissions.*construction.*CO2",
        "Rubber and Plastics Products": "BAU Industrial Sector Pollutant Emissions.*rubber and plastic products.*CO2"
    }

    # 计算工业非氢部门
    industryCO2_data = data[
        data['Time'].str.contains("BAU Industrial Sector Pollutant Emissions") & data['Time'].str.contains("CO2") & ~
        data['Time'].str.contains("agriculture")]
    ...
    industryP_data = data[
        data['Time'].str.contains("BAU Value Added by ISIC Code by Year") & data['Time'].str.contains(pattern)]
...
def plot_sector_CO2_intensity(processed_data, output_folder, language):
    ...
    output_path = f"{output_folder}/图11：行业二氧化碳排放强度{language}无标题.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"{titles[language]} chart saved to: {output_path}")

    plt.title(titles[language],size=18)
    output_path = f"{output_folder}/图11：行业二氧化碳排放强度{language}.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"{titles[language]} chart saved to: {output_path}")
