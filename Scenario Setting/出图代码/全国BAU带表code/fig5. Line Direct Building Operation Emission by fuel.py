...
def read_and_prepare_data(file_path):
    ...
    sectors = {
        ...
        "petroleum diesel": "Direct Buildings Pollutant Emissions.*(urban residential|rural residential|commercial).*petroleum diesel.*CO2"
    }
...
def plot_sector_energy_consumption(processed_data, output_folder, language):
    ...
    sector_labels = {
        ...
        "petroleum diesel": "Petroleum" if language == 'en' else " 柴油"
    }
    ...
    output_path = f"{output_folder}/图5：折线图建筑部门分燃料直接碳排放{language}无标题.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"{titles[language]} chart saved to: {output_path}")
    
    plt.title(titles[language], fontsize=18)
    output_path = f"{output_folder}/图5：折线图建筑部门分燃料直接碳排放{language}.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"{titles[language]} chart saved to: {output_path}")
