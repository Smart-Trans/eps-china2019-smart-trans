...
start_year_sectors = {
    ...
    "Wind": "SYC Start Year Electricity Generation Capacity.*(onshore wind|off wind).",
...
bau_sectors = {
    "Coal": "BAU Electricity Generation Capacity.*(hard coal|lignite es).",
    "Natural Gas": "BAU Electricity Generation Capacity.*(natural gas|natural gas peaker es).",
    "Hydro": "BAU Electricity Generation Capacity.*hydro",
    "Wind": "BAU Electricity Generation Capacity.*(onshore wind|off wind).",
    "Solar": "BAU Electricity Generation Capacity.*(solar PV|solar thermal).",
    "Other": "BAU Electricity Generation Capacity.*(geothermal|biomass es|municipal solid waste).",
    "Pumped Hydro": "BPHC BAU Pumped Hydro Capacity",
    "Battery Storage" : "BAU Grid Battery Storage Capacity in MWh",
    "Demand Response" : "BAU This Year Demand Response Capacity"
}
...
def read_and_prepare_data(file_path):
    ...
    for sector_key, pattern in start_year_sectors.items():
     ...
     # Retrieve data for selected years for all sectors except 'Pumped Hydro'
    for sector_key, pattern in bau_sectors.items():
         if (sector_key != 'Pumped Hydro' and sector_key != 'Battery Storage' and sector_key != 'Demand Response' ) :  # Skip 'Pumped Hydro' here
             sector_data = data[data['Time'].str.contains(pattern, regex=True) &~data['Time'].str.contains("Change") &~data['Time'].str.contains("Output") &~data['Time'].str.contains("Surviving") &~data['Time'].str.contains("Shifted") &~data['Time'].str.contains("Year") &~data['Time'].str.contains("Distributed") &~data['Time'].str.contains("before")]
    ...
    if 'Pumped Hydro' in bau_sectors:
         ...
         # Starting from the 12th column as the year 2023
         for i, year in enumerate(selected_years):
             processed_data.at[year, 'Pumped Hydro'] = pumped_hydro_data.iloc[:, 11 + i].apply(pd.to_numeric, errors='coerce').fillna(0).sum()
    if 'Battery Storage' in bau_sectors:
         battery_storage_pattern=bau_sectors['Battery Storage']
         battery_storage_data=data[data['Time'].str.contains(battery_storage_pattern, regex=True)]
         for year in selected_years:
             processed_data.at[year, sector_key] = sector_data[year].apply(pd.to_numeric, errors='coerce').fillna(0).sum()
             
    if 'Demand Response' in bau_sectors:
         demand_response_pattern=bau_sectors['Demand Response']
         demand_response_data=data[data['Time'].str.contains(demand_response_pattern, regex=True)]
         for year in selected_years:
             processed_data.at[year, sector_key] = sector_data[year].apply(pd.to_numeric, errors='coerce').fillna(0).sum()
...
def plot_sector_energy_consumption(processed_data, output_folder, language, legend_titles):
    ...
    output_path = f"{output_folder}/图9：装机量{language}无标题.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"Chart saved to: {output_path}")
    
    plt.title(titles[language], size=18, fontname=font_name)
    output_path = f"{output_folder}/图9：装机量{language}.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"Chart saved to: {output_path}")
