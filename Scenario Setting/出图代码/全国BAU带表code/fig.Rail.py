...
start_year_sectors = {
    "Gasoline": "SYVbT Start Year Vehicles by Technology.*rail.*gasoline vehicle.",
    "Diesel": "SYVbT Start Year Vehicles by Technology.*rail.*diesel vehicle.",
    "Natural Gas": "SYVbT Start Year Vehicles by Technology.*rail.*natural gas vehicle.",
    "LPG": "SYVbT Start Year Vehicles by Technology.*rail.*LPG vehicle.",
    "PHV": "SYVbT Start Year Vehicles by Technology.*rail.*plugin hybrid vehicle.",
    "BEV": "SYVbT Start Year Vehicles by Technology.*rail.*battery electric vehicle.",
    "Hydrogen": "SYVbT Start Year Vehicles by Technology.*rail.*hydrogen vehicle."
}

bau_sectors = {
    "Gasoline": "BAU Vehicles.*rail.*gasoline vehicle.",
    "Diesel": "BAU Vehicles.*rail.*diesel vehicle.",
    "Natural Gas": "BAU Vehicles.*rail.*natural gas vehicle.",
    "LPG": "BAU Vehicles.*rail.*LPG vehicle.",
    "PHV": "BAU Vehicles.*rail.*plugin hybrid vehicle.",
    "BEV": "BAU Vehicles.*rail.*battery electric vehicle.",
    "Hydrogen": "BAU Vehicles.*rail.*hydrogen vehicle."
}
...
def plot_sector_energy_consumption(processed_data, output_folder):
    ...
    # yticks=range(0,16000,2000)
    # plt.yticks(yticks)
    plt.rcParams['font.sans-serif'] = ['Times New Roman']

    output_path = f"{output_folder}/Rail无标题.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"Chart saved to: {output_path}")
    
    plt.title(titles,size=18)
    output_path = f"{output_folder}/Rail.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"Chart saved to: {output_path}")
