...
def read_and_prepare_data(file_path):
    ...
    sectors = {
        ...
        #"Metal Products": "BAU Industrial Sector Pollutant Emissions.*metal products except machinery and vehicles.*CO2"
    ...
    total_industrial_emissions = data[data['Time'].str.contains("BAU Industrial Sector Pollutant Emissions.*CO2")][
...
def plot_sector_energy_consumption(processed_data, output_folder, language):
    ...
    output_path = f"{output_folder}/图11：分行业二氧化碳排放{language}无标题1.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"{titles[language]} chart saved to: {output_path}")
    
    plt.title(titles[language],size=18)
    output_path = f"{output_folder}/图11：分行业二氧化碳排放{language}.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"{titles[language]} chart saved to: {output_path}")
