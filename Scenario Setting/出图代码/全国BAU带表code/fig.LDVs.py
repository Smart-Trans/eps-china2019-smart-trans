...
bau_sectors = {
    "Gasoline": "BAU Vehicles.*LDVs.*gasoline vehicle.",
    "Diesel": "BAU Vehicles.*LDVs.*diesel vehicle.",
    "Natural Gas": "BAU Vehicles.*LDVs.*natural gas vehicle.",
    "LPG": "BAU Vehicles.*LDVs.*LPG vehicle.",
    "PHV": "BAU Vehicles.*LDVs.*plugin hybrid vehicle.",
    "BEV": "BAU Vehicles.*LDVs.*battery electric vehicle.",
    "Hydrogen": "BAU Vehicles.*LDVs.*hydrogen vehicle."
}
...
def plot_sector_energy_consumption(processed_data, output_folder):
    ...
    output_path = f"{output_folder}/LDVs无标题.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"Chart saved to: {output_path}")

    plt.title(titles,size=18)
    output_path = f"{output_folder}/LDVs.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"Chart saved to: {output_path}")
