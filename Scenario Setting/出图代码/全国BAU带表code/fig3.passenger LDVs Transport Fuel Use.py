...
def read_and_prepare_data(file_path):
    ...
    sectors = {
        ...
        "electricity": "BAU Transportation Sector Fuel Used.*LDVs.*passenger.*electricity",
        #"heavy or residual fuel oil": "BAU Transportation Sector Fuel Used.*LDVs.*passenger.*heavy or residual fuel oil",
        "hydrogen": "BAU Transportation Sector Fuel Used.*LDVs.*passenger.*hydrogen*",
        "jet fuel": "BAU Transportation Sector Fuel Used.*LDVs.*passenger.*jet fuel",
        "natural gas": "BAU Transportation Sector Fuel Used.*LDVs.*passenger.*natural gas",
        #"LPG propane or butane": "BAU Transportation Sector Fuel Used.*LDVs.*passenger.*LPG propane or butane",
        "petroleum diesel": "BAU Transportation Sector Fuel Used.*LDVs.*passenger.*petroleum diesel",
        "petroleum gasoline": "BAU Transportation Sector Fuel Used.*LDVs.*passenger.*petroleum gasoline"
    }
...
def plot_sector_energy_consumption(processed_data, output_folder, language):
    ...
    output_path = f"{output_folder}/图3：载客轻型车运输燃料消费{language}无标题.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"{titles[language]} chart saved to: {output_path}")
    
    plt.title(titles[language],size=18)
    output_path = f"{output_folder}/图3：载客轻型车运输燃料消费{language}.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"{titles[language]} chart saved to: {output_path}")
