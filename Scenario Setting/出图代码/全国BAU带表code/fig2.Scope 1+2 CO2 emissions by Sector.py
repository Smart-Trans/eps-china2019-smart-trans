...
def read_and_prepare_data(file_path):
    ...
    #########用电量数据
    transportation_electricity_data=search(data,'Time',"BAU Transportation Sector Electricity Demand")
    transportation_electricity=Sum(transportation_electricity_data, year_columns)
    industrial_electricity_data=search(data,'Time',"BAU Industrial Sector Electricity Demand")
    industrial_electricity=Sum(industrial_electricity_data,year_columns)
    hydro_electricity_data=search(data,'Time',"BAU Hydrogen Sector Electricity Demand")
    hydro_electricity=Sum(hydro_electricity_data,year_columns)
    heat_electricity_data=search(data,'Time',"BAU District Heat Electricity Demand")
    ...
    import_electricity_data=search(data,'Time',"BAU Net Imports of Electricity")
    ...
    ###########交通数据获取
    #Direct
    Direct_transportation_data=search(data,'Time',"BAU Total Pollutant Emissions by Sector\[transportation sector,CO2\]")
    Direct_transportation=Sum(Direct_transportation_data,year_columns)
    
    #From Electricity
    Direct_electricity_data=search(data,'Time',"BAU Total Pollutant Emissions by Sector\[electricity sector,CO2\]")
    #计算交通用电量占比
    transportation_percent=transportation_electricity / total_electricity
    ...
    ###########居民建筑数据获取
    #Direct
    Direct_residential_data=search(data,'Time',"BAU Total Pollutant Emissions by Sector","residential","CO2")
    ...
    ###########公共建筑数据获取
    #Direct
    Direct_commercial_data=search(data,'Time',"BAU Total Pollutant Emissions by Sector","commercial","CO2")
    ...
    ###########工业过程数据获取
    #Indirect
    indirect_process_data=search(data,'Time',"BAU Process Emissions after CCS","CO2")
    ...
    ###########工业燃料数据获取
    #Direct
    industry_data=search(data,'Time',"BAU Total Pollutant Emissions by Sector","industry","CO2")
    industry=Sum(industry_data,year_columns)
    Direct_fuel=industry-Indirect_process
    
    #From Electricity
    fuel_electricity_data=search(data,'Time',"BAU Industrial Sector Electricity Demand")
    #计算工业燃料用电量占比
    ...
    ###########外送电力数据获取
    #发电量
    Electricity_import_data=search(data,'Time',"BAU Net Imports of Electricity")
...
def plot_stacked_area_chart(plot_data, year_columns, output_folder, language='en'):
    ...
    ncols = {'en': 3, 'cn': 3}
    colors = ["#c01b00", "#000000", "#969696", "#f1bb18", "#087bf1", "#004185", "#b07aa1", "#ff9da7"]
    if language == 'cn':
    ...
    if language == 'en':
        for label, data in plot_data.items():
            ...
            label = sector_labels[language][label]
            labels.append(label)
            tabletoshow[label] = list(data*100)
    ...
    # 确定y轴最大值
    # yticksup=max(stack_baseline) + 2
    # if (yticksup % 2 != 0):
    #     yticksup+=1
    # plt.yticks(np.arange(0, yticksup, step=2),size=14)
    ...
    output_path = f"{output_folder}/图2：分部门二氧化碳直接排放（范围1+2）{language}无标题.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"Chart saved to: {output_path}")
    
    plt.title(titles[language],size=18)
    output_path = f"{output_folder}/图2：分部门二氧化碳直接排放（范围1+2）{language}.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"Chart saved to: {output_path}")
