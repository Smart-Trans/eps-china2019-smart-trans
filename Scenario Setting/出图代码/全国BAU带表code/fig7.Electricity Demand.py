...
def read_and_prepare_data(file_path):
    ...
    sectors = {
        "Transportation": "BAU Transportation Sector Electricity Demand",
        "Industrial": "BAU Industrial Sector Electricity Demand",
        "Hydrogen": "BAU Hydrogen Sector Electricity Demand",
        "Export": "BAU Net Imports of Electricity",
...
def plot_sector_energy_consumption(processed_data, output_folder, language):
    ...
    output_path = f"{output_folder}/图7：分部门用电量{language}无标题.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"{titles[language]} chart saved to: {output_path}")
    
    plt.title(titles[language],size=18)
    output_path = f"{output_folder}/图7：分部门用电量{language}.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"{titles[language]} chart saved to: {output_path}")
