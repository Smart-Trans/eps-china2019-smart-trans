...
start_year_sectors = {
    "Gasoline": "SYVbT Start Year Vehicles by Technology.*ships.*gasoline vehicle.",
    "Diesel": "SYVbT Start Year Vehicles by Technology.*ships.*diesel vehicle.",
    "Natural Gas": "SYVbT Start Year Vehicles by Technology.*ships.*natural gas vehicle.",
    "LPG": "SYVbT Start Year Vehicles by Technology.*ships.*LPG vehicle.",
    "PHV": "SYVbT Start Year Vehicles by Technology.*ships.*plugin hybrid vehicle.",
    "BEV": "SYVbT Start Year Vehicles by Technology.*ships.*battery electric vehicle.",
    "Hydrogen": "SYVbT Start Year Vehicles by Technology.*ships.*hydrogen vehicle."
}

bau_sectors = {
    "Gasoline": "BAU Vehicles.*ships.*gasoline vehicle.",
    "Diesel": "BAU Vehicles.*ships.*diesel vehicle.",
    "Natural Gas": "BAU Vehicles.*ships.*natural gas vehicle.",
    "LPG": "BAU Vehicles.*ships.*LPG vehicle.",
    "PHV": "BAU Vehicles.*ships.*plugin hybrid vehicle.",
    "BEV": "BAU Vehicles.*ships.*battery electric vehicle.",
    "Hydrogen": "BAU Vehicles.*ships.*hydrogen vehicle."
}
...
def plot_sector_energy_consumption(processed_data, output_folder):
    ...
    output_path = f"{output_folder}/Ships无标题.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"Chart saved to: {output_path}")
    
    plt.title(titles,size=18)
    output_path = f"{output_folder}/Ships.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"Chart saved to: {output_path}")
