...
def read_and_prepare_data(file_path):
    ...
    sectors = {
        "electricity": "Transportation Sector Fuel Used.*freight.*electricity",
        "heavy or residual fuel oil": "Transportation Sector Fuel Used.*freight.*heavy or residual fuel oil",
        "hydrogen": "Transportation Sector Fuel Used.*freight.*hydrogen*",
        "jet fuel": "Transportation Sector Fuel Used.*freight.*jet fuel",
        "LPG propane or butane": "Transportation Sector Fuel Used.*freight.*LPG propane or butane",
        "natural gas": "Transportation Sector Fuel Used.*freight.*natural gas",
        "petroleum diesel": "Transportation Sector Fuel Used.*freight.*petroleum diesel",
        "petroleum gasoline": "Transportation Sector Fuel Used.*freight.*petroleum gasoline"
    }
...
def plot_sector_energy_consumption(processed_data, output_folder, language):
    ...
    output_path = f"{output_folder}/交通-能耗：载货运输燃料消费{language}无标题.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"{titles[language]} chart saved to: {output_path}")
    
    plt.title(titles[language], fontsize=18)
    output_path = f"{output_folder}/交通-能耗：载货运输燃料消费{language}.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"{titles[language]} chart saved to: {output_path}")
