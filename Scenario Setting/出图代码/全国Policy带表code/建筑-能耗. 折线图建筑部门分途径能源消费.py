...
sectors = {
    "heating": "Building Components Energy Use.*heating.",
    "cooling and ventilation": "Building Components Energy Use.*cooling and ventilation.",
    "lighting": "Building Components Energy Use.*lighting.",
    "appliances": "Building Components Energy Use.*appliance.",
    "other component": "Building Components Energy Use.*other component."
}
...
def plot_sector_energy_consumption(processed_data, output_folder, language):
    ...
    for sector_key in sectors.keys():
        axs[1].fill_between(
            processed_data.index.astype(int),
            bottom,
            bottom+processed_data[sector_key],
            color=colors[sector_key],
            label=sector_labels[sector_key]# Set bar width here
        )
    ...
    output_path = f"{output_folder}/建筑-能耗：折线图建筑部门分途径能源消费{language}无标题.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"Chart saved to: {output_path}")
    
    plt.title(titles[language], fontsize=18)
    output_path = f"{output_folder}/建筑-能耗：折线图建筑部门分途径能源消费{language}.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"Chart saved to: {output_path}")
