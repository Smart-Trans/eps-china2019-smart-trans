...
def read_and_prepare_data(file_path):
    ...
    sectors = {
        "biomass": "Building Components Energy Use.*biomass.",
        "electricity": "Building Components Energy Use.*electricity.",
        "hard coal": "Building Components Energy Use.*hard coal.",
        "heat": "Building Components Energy Use.*heat bf.",
        "LPG propane or butane": "Building Components Energy Use.*LPG propane or butane.",
        "natural gas": "Building Components Energy Use.*natural gas.",
        "petroleum": "Building Components Energy Use.*petroleum."
    }
...
def plot_sector_energy_consumption(processed_data, output_folder, language):
    # 在英文图表中进行单位转换
    if language == 'en':
        processed_data *= (10**-12 / 3.60087861438191E-12)
    else:
    ...
    sector_labels = {
        ...
        "petroleum": "Petroleum" if language == 'en' else "石油"
    }
    ...
    for sector in processed_data.columns:
        ...
        yrange = max(yrange,max(processed_data[sector]+bottom))
    ...
    output_path = f"{output_folder}/建筑-能耗：折线图建筑部门分品种能源消费{language}无标题.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"{titles[language]} chart saved to: {output_path}")
    
    plt.title(titles[language], fontsize=18)
    output_path = f"{output_folder}/建筑-能耗：折线图建筑部门分品种能源消费{language}.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"{titles[language]} chart saved to: {output_path}")
