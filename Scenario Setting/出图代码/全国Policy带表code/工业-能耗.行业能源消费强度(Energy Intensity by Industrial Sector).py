...
def read_and_prepare_data(file_path):
    ...
    #计算工业非氢部门
    industryE_data=data[data['Time'].str.contains("Industrial Fuel Use for Energy Purposes") & ~data['Time'].str.contains("BAU") & ~data['Time'].str.contains("agriculture") & ~data['Time'].str.contains("Change") & ~data['Time'].str.contains("Output") & ~data['Time'].str.contains("before")]
    ...
    for sector_key, pattern in energy_sectors.items():
        # 确保行不包含“BAU”
        sector_data = data[data['Time'].str.contains(pattern, regex=True) & ~data['Time'].str.contains("BAU") & ~data['Time'].str.contains("Output") & ~data['Time'].str.contains("Change") & ~data['Time'].str.contains("before")]
...
def plot_sector_energy_consumption(processed_data, output_folder, language):
    ...
    output_path = f"{output_folder}/工业-能耗：行业能源消费强度{language}无标题.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"{titles[language]} chart saved to: {output_path}")

    plt.title(titles[language],size=18)
    output_path = f"{output_folder}/工业-能耗：行业能源消费强度{language}.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"{titles[language]} chart saved to: {output_path}")
