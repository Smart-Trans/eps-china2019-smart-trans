...
def plot_stacked_area_chart(BAU_plot_data, Policy_plot_data, year_columns, output_folder, language='en'):
    ...
    titles = {'en': 'BAU vs CNS Net Greenhouse Gas Emission by Sector', 'cn': 'BAU vs CNS 温室气体直接净排放'}
    ...
    plt.xticks(xticks)
    plt.legend(loc=8, ncol=ncols[language], bbox_to_anchor=(0.5, -0.5), edgecolor="white")
    ...
    output_path = f"{output_folder}/全行业-碳排放.BAU与CNS温室气体对比图无标题{language}.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"Chart saved to: {output_path}")

    plt.title(titles[language], size=18)
    output_path = f"{output_folder}/全行业-碳排放.BAU与CNS温室气体对比图{language}.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"Chart saved to: {output_path}")
    plt.close()
