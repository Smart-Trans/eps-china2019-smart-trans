...
def read_and_prepare_data(file_path):
    ...
    process_emissions_filter = data['Time'].str.contains("Process Emissions after CCS") & data['Time'].str.contains("CO2") & ~data['Time'].str.contains("BAU")
    ...
    industry_sector_data = data[data['Time']=="Total Pollutant Emissions by Sector[industry sector,CO2]"]
    industry_sector=Sum(industry_sector_data,year_columns)
    
    # 计算 policy Industrial Process
    policy_industrial_process = process_emissions_sum

    # 计算 policy Industrial Fuel
    policy_industrial_fuel = industry_sector - policy_industrial_process
    ...
    plot_data = pd.DataFrame({
        'transportation sector': policy_transport.values * 1e-14,
        ...
        'electricity sector': policy_electricity.values * 1e-14,
        'heating and H2':policy_heat.values * 1e-14,
        'residential buildings sector': policy_residential.values * 1e-14,
        'commercial buildings sector': policy_commercial.values * 1e-14,
        'Non CO2': policy_NonCO2.values * 1e-14
    }, index=year_columns)
...
def plot_stacked_area_chart(plot_data, year_columns, output_folder, language='en'):
    ...
    titles = {'en': 'Scope 1 CO2 Emission by Sector', 'cn': '分部门二氧化碳直接排放（范围1）'}
    ylabels = {'en': 'Mt CO2', 'cn': '排放量 (亿吨)'}
    sector_labels = {
        'en': {
            'policy Industrial Process': 'Industrial Process',
            'policy Industrial Fuel': 'Industrial Fuel',
            'transportation sector': 'Transportation',
            'electricity sector': 'Electricity',
            'residential buildings sector': 'Residential Bldgs',
            'commercial buildings sector': 'Commercial Bldgs',
            'heating and H2': 'Heating and H2',
            'Non CO2': 'Non CO2'
        },
        'cn': {
            'policy Industrial Process': "工业过程",
            'policy Industrial Fuel': "工业燃料",
            'transportation sector': "交通部门",
            'electricity sector': "电力部门",
            'residential buildings sector': "居民建筑",
            'commercial buildings sector': "公共建筑",
    ...
    output_path = f"{output_folder}/全行业-碳排放：分部门二氧化碳直接排放（范围1）{language}无标题.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"Chart saved to: {output_path}")
    
    plt.title(titles[language],size=18)
    output_path = f"{output_folder}/全行业-碳排放：分部门二氧化碳直接排放（范围1）{language}.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"Chart saved to: {output_path}")
...
if __name__ == "__main__":
    ...
    #output_folder = "C:\\Users\\Mywljy\\Desktop\\Policy\\"
