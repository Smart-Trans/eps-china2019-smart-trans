...
sectors = {
    "rural residential": "Building Components Energy Use.*rural residential.",
    "urban residential": "Building Components Energy Use.*urban residential.",
    "commercial": "Building Components Energy Use.*commercial."
}
...
def read_and_prepare_data(file_path):
    ...
    for sector_key, pattern in sectors.items():
        # Filter to exclude entries containing 'BAU'
...
def plot_sector_energy_consumption(processed_data, output_folder, language):
    ...
    if language == 'en':
        processed_data *= (10**-12 / 3.60087861438191E-12) # 单位转换
    ...
    colors = ["#00b050", "#969696", "#004185", "#76b7b2", "#59a14f", "#edc948", "#b07aa1", "#ff9da7"]
    ...
    labels = []
    yrange = 0
    for sector, color in zip(sector_labels.keys(), colors):
        axs[1].fill_between(processed_data.index.astype(int), bottom,bottom+processed_data[sector], color=color, label=sector_labels[sector])
    ...
    output_path = f"{output_folder}/建筑-能耗：折线图建筑部门分建筑类型能源消费{language}无标题.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"{titles[language]} chart saved to: {output_path}")
    
    plt.title(titles[language], fontsize=18)
    output_path = f"{output_folder}/建筑-能耗：折线图建筑部门分建筑类型能源消费{language}.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"{titles[language]} chart saved to: {output_path}")
