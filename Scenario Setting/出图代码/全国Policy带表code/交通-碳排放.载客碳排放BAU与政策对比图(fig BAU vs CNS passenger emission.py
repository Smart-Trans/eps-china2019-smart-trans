...
def read_and_prepare_data(file_path):
    ...
    BAU_sources = {
        "BAU Aircraft": "BAU Transportation Pollutant Emissions Disaggregated.*aircraft.*CO2.*passenger",
        "BAU HDVs": "BAU Transportation Pollutant Emissions Disaggregated.*HDVs.*CO2.*passenger",
        "BAU LDVs": "BAU Transportation Pollutant Emissions Disaggregated.*LDVs.*CO2.*passenger"
    }
    policy_sources = {
        "Policy Aircraft": "Transportation Pollutant Emissions Disaggregated.*aircraft.*CO2.*passenger",
        "Policy HDVs": "Transportation Pollutant Emissions Disaggregated.*HDVs.*CO2.*passenger",
        "Policy LDVs": "Transportation Pollutant Emissions Disaggregated.*LDVs.*CO2.*passenger"
    }
...
def plot_stacked_area_chart(BAU_plot_data,Policy_plot_data, year_columns, output_folder,language):
    ...
    if language=='en':
     titles = 'BAU vs CNS Passenger CO2 Emission'
    else:
     titles = '载客二氧化碳排放量对比'
    ...
    if language=='cn':
     sector_labels = {
            "BAU HDVs": "BAU载客重型车",
            "BAU Aircraft": "BAU载客飞机",
            "BAU LDVs": "BAU载客轻型车",
            "Policy HDVs": "CNS载客重型车",
            "Policy Aircraft": "CNS载客飞机",
            "Policy LDVs": "CNS载客轻型车"
    }
    else:
     sector_labels = {
               "BAU HDVs": "BAU Passenger HDVs",
               "BAU Aircraft": "BAU Passenger Aircraft",
               "BAU LDVs": "BAU Passenger LDVs",
               "Policy HDVs": "CNS Passenger HDVs",
               "Policy Aircraft": "CNS Passenger Aircraft",
               "Policy LDVs": "CNS Passenger LDVs"
       }
    colors={
            "BAU HDVs": "#000000",
            "BAU Aircraft": "#c2dffd",
    ...
    output_path = f"{output_folder}/交通-碳排放：载客碳排放BAU与政策对比图无标题{language}.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"Chart saved to: {output_path}")
    
    plt.title(titles,size=18)
    output_path = f"{output_folder}/交通-碳排放：载客碳排放BAU与政策对比图{language}.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"Chart saved to: {output_path}")
    plt.close()
