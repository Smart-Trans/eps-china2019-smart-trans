...
def read_and_prepare_data(file_path):
    ...
    process_emissions_filter = data['Time'].str.contains("Process Emissions after CCS") & data['Time'].str.contains(
        "CO2") & ~data['Time'].str.contains("BAU")
    ...
    policy_bldg = policy_residential + policy_commercial
    ...
    plot_data = pd.DataFrame({
        ...
        'buildings sector': policy_bldg.values * 1e-14,
        'Non CO2': policy_NonCO2.values * 1e-14,
        'land use': policy_LULUCF.values * 1e-14
    }, index=year_columns)
...
def plot_stacked_area_chart(plot_data, year_columns, output_folder, language='en'):
    ...
    if language == 'en':
        plt.yticks(ticks=[-2500, 0, 2500, 5000, 7500, 10000, 12500, 15000],
                   labels=['-2500', '0', '2500', '5000', '7500', '10000', '12500', '15000'])
    else:
        plt.yticks(ticks=[-25, 0, 25, 50, 75, 100, 125, 150],
                   labels=['-25', '0', '25', '50', '75', '100', '125', '150'])
    ...
    output_path = f"{output_folder}/全行业-碳排放：CNS温室气体净排放{language}无标题.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"Chart saved to: {output_path}")

    plt.title(titles[language], size=18)
    output_path = f"{output_folder}/全行业-碳排放：CNS温室气体净排放{language}.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"Chart saved to: {output_path}")
...
if __name__ == "__main__":
    ...
    #file_path = "D:\\实习\\EPS\\EPS数据\\全国\\11.13全国CNS.csv"
    #output_folder = "C:\\Users\\Mywljy\\Desktop\\渐进转型政策\\"
