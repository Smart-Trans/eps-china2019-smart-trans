...
def read_and_prepare_data(file_path):
    ...
    sectors = {
        "Hard Coal": "Electricity Generation by Type.*hard coal.",
        "Hydro": "Electricity Generation by Type.*hydro.",
        "Onshore Wind": "Electricity Generation by Type.*onshore wind.",
        "Solar PV": "Electricity Generation by Type.*solar PV.",
        "Biomass": "Electricity Generation by Type.*biomass.",
        "MSW": "Electricity Generation by Type.*municipal solid waste.",
        "Solar Thermal": "Electricity Generation by Type.*solar thermal.",
        "Distributed": "BDEQ BAU Electricity Output from Distributed Sources"  # 假设已经是没有 BAU 的数据
    }

    for sector_key, pattern in sectors.items():
        sector_data = data[data['Time'].str.contains(pattern, regex=True) & ~data['Time'].str.contains("BAU") & ~data['Time'].str.contains("Change") & ~data['Time'].str.contains("Year") & ~data['Time'].str.contains("Output") &  ~data['Time'].str.contains("Distributed")]
...
def plot_sector_energy_consumption(processed_data, output_folder, language):
    ...
    output_path = f"{output_folder}/其他：分品种发电量{language}无标题.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"{titles[language]} chart saved to: {output_path}")
    
    plt.title(titles[language],size=18)
    output_path = f"{output_folder}/其他：分品种发电量{language}.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"{titles[language]} chart saved to: {output_path}")
