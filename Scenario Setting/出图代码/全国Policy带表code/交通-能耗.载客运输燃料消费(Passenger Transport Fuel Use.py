...
def read_and_prepare_data(file_path):
    ...
    electricity_data=search(data,'Time',"Transportation Sector Fuel Used","electricity","passenger")
    electricity=Sum(electricity_data,year_columns)
    
    jet_fuel_data=search(data,'Time',"Transportation Sector Fuel Used","jet fuel","passenger")
    jet_fuel=Sum(jet_fuel_data,year_columns)
    
    LPG_data=search(data,'Time',"Transportation Sector Fuel Used","LPG propane or butane","passenger")
    LPG=Sum(LPG_data,year_columns)
    
    natural_gas_data=search(data,'Time',"Transportation Sector Fuel Used","natural gas tf","passenger")
    natural_gas=Sum(natural_gas_data,year_columns)
    
    petroleum_diesel_data=search(data,'Time',"Transportation Sector Fuel Used","petroleum diesel tf","passenger")
    petroleum_diesel=Sum(petroleum_diesel_data,year_columns)
    
    petroleum_gasoline_data=search(data,'Time',"Transportation Sector Fuel Used","petroleum gasoline tf","passenger")
    ...
    processed_data=pd.DataFrame({
        ...
        "LPG propane or butane": LPG * 3.60087861438191E-12,
...
def plot_sector_energy_consumption(processed_data, output_folder, language):
    ...
    output_path = f"{output_folder}/交通-能耗：载客运输燃料消费{language}无标题.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"{titles[language]} chart saved to: {output_path}")
    
    plt.title(titles[language],size=18)
    output_path = f"{output_folder}/交通-能耗：载客运输燃料消费{language}.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"{titles[language]} chart saved to: {output_path}")
