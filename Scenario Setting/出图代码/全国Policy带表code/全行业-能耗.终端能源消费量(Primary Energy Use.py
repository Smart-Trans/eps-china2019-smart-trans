...
def read_and_prepare_data(file_path):
    ...
    sectors = {
        "Transportation": "Total Energy Use by Sector.*transportation",
        "Residential": "Total Energy Use by Sector.*residential",
        "Commercial": "Total Energy Use by Sector.*commercial",
    }
    ...
    # 工业燃料计算
    industry_pattern = "Total Energy Use by Sector.*industry"
    industry_data = data[data['Time'].str.contains(industry_pattern, regex=True) & ~data['Time'].str.contains('BAU', regex=True)]
    industry_sum = industry_data[year_columns].apply(pd.to_numeric, errors='coerce').fillna(0).sum()

    # 非燃料使用
    non_energy_pattern = "Industrial Fuel Use for Non Energy Purposes"
    non_energy_data = data[data['Time'].str.contains(non_energy_pattern, regex=True) & ~data['Time'].str.contains('BAU', regex=True)]
    print(non_energy_data.head)
    print(non_energy_data.tail)
    ...
    # 排除特定模式的非燃料使用
    exclusions_patterns = [
        "Industrial Fuel Use for Non Energy Purposes\[coal mining 05,hard coal if\]",
        "Industrial Fuel Use for Non Energy Purposes\[refined petroleum and coke 19,hard coal if\]",
        "Industrial Fuel Use for Non Energy Purposes\[refined petroleum and coke 19,crude oil if\]"
    ]
...
def plot_sector_energy_consumption(processed_data, output_folder, language):
    ...
    output_path = f"{output_folder}/全行业-能耗：终端能源消费量{language}无标题.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"{titles[language]} chart saved to: {output_path}")
    
    plt.title(titles[language], fontsize=18)
    output_path = f"{output_folder}/全行业-能耗：终端能源消费量{language}.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"{titles[language]} chart saved to: {output_path}")
