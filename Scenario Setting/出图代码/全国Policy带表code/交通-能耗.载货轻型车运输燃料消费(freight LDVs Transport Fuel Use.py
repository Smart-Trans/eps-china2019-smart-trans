...
def read_and_prepare_data(file_path):
    ...
    sectors = {
        "Electricity": "Transportation Sector Fuel Used.*LDVs.*freight.*electricity",
       # "heavy or residual fuel oil": "Transportation Sector Fuel Used.*LDVs.*freight.*heavy or residual fuel oil",
        "Hydrogen": "Transportation Sector Fuel Used.*LDVs.*freight.*hydrogen*",
        "Jet Fuel": "Transportation Sector Fuel Used.*LDVs.*freight.*jet fuel",
       # "LPG Propane or Butane": "Transportation Sector Fuel Used.*LDVs.*freight.*LPG propane or butane",
        "Natural Gas": "Transportation Sector Fuel Used.*LDVs.*freight.*natural gas",
        "Petroleum Diesel": "Transportation Sector Fuel Used.*LDVs.*freight.*petroleum diesel",
        "Petroleum Gasoline": "Transportation Sector Fuel Used.*LDVs.*freight.*petroleum gasoline",
        "Biofuel Diesel": "Transportation Sector Fuel Used.*LDVs.*freight.*biofuel diesel",
        "Biofuel Gasoline": "Transportation Sector Fuel Used.*LDVs.*freight.*biofuel gasoline"
    }

    for sector_key, pattern in sectors.items():
        print(f"Processing sector: {sector_key}")
        # Exclude lines containing 'BAU'
        sector_data = data[data['Time'].str.contains(pattern, regex=True) & ~data['Time'].str.contains('BAU', regex=True)  & ~data['Time'].str.contains('Year', regex=True)]
...
def plot_sector_energy_consumption(processed_data, output_folder, language):
    ...
    output_path = f"{output_folder}/交通-能耗：载货轻型车运输燃料消费{language}无标题.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"{titles[language]} chart saved to: {output_path}")
    
    plt.title(titles[language],size=18)
    output_path = f"{output_folder}/交通-能耗：载货轻型车运输燃料消费{language}.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"{titles[language]} chart saved to: {output_path}")
