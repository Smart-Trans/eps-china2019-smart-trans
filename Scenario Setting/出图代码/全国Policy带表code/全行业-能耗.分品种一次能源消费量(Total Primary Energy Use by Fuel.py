...
def read_and_prepare_data(file_path):
    ...
    petroleum_sources = ['diesel', 'LPG', 'gasoline', 'fuel oil', 'jet fuel', 'crude oil', 'biofuel gasoline', 'biofuel diesel']
...
def plot_energy_consumption(processed_data, output_folder, language):
    ...
    # 保存图表
    output_path = f"{output_folder}/全行业-能耗：分品种一次能源消费量{language}无标题.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"{titles[language]} chart saved to: {output_path}")
    
    plt.title(titles[language], fontsize=18)
    output_path = f"{output_folder}/全行业-能耗：分品种一次能源消费量{language}.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"{titles[language]} chart saved to: {output_path}")
