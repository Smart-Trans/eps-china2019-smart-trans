...
def read_and_prepare_data(file_path):
    ...
    sectors = {
        "LDVs freight": "Transportation Pollutant Emissions Disaggregated.*LDVs.*CO2.*freight.",
        "HDVs passenger": "Transportation Pollutant Emissions Disaggregated.*HDVs.*CO2.*passenger.",
        "HDVs freight": "Transportation Pollutant Emissions Disaggregated.*HDVs.*CO2.*freight.",
        "LDVs passenger": "Transportation Pollutant Emissions Disaggregated.*LDVs.*CO2.*passenger."
    }
...
def plot_sector_energy_consumption(processed_data, output_folder, language):
    ...
    axs[1].yaxis.set_label_coords(0.05, 1.01)  # 通过坐标来微调标签位置
    ...
    axs[0].text(0.5, -0.3, f"year: {max_year},max: {max_value:.1f}", va='center', ha='center', fontsize=12)
    output_path = f"{output_folder}/交通-碳排放：道路汽车二氧化碳排放{language}无标题.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"{titles[language]} chart saved to: {output_path}")
    
    plt.title(titles[language], fontsize=18)
    output_path = f"{output_folder}/交通-碳排放：道路汽车二氧化碳排放{language}.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"{titles[language]} chart saved to: {output_path}")
