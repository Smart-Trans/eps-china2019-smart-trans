...
def plot_stacked_area_chart(BAU_plot_data,Policy_plot_data, year_columns, output_folder,language):
    ...
    if language =='cn':
     ...
     # plt.yticks(yticks)
    # axs[1].set_ylim(bottom=0,top=12)
    #else:
     # yticks=range(0,30,5)
     # plt.yticks(yticks) 
    ...
    output_path = f"{output_folder}/全行业-碳排放：载货碳排放BAU与政策对比图无标题{language}.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"Chart saved to: {output_path}")
    
    plt.title(titles,size=18)
    output_path = f"{output_folder}/全行业-碳排放：载货碳排放BAU与政策对比图{language}.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"Chart saved to: {output_path}")
    plt.close()
